...
Run the Gaiden_Randomizer.exe from within that folder
    *NOTE: if you can't run the EXE for whatever reason, the order of operations to create a rando seed is: shuffle.py, replace_items.py, convert.py. The EXE just runs those three scripts back to back. You may also be able to just run the gaiden_randomizer.py as it should do the same thing.
